...
class MyStack:
    ...
    def push(self, x: int) -> None:
        ...
        # transfers existing values behind the new value
        while self.main_queue:
            self.auxiliary_queue.append(self.main_queue.popleft())

        # swaps the queue references; the auxiliary queue becomes empty helper storage
        self.main_queue, self.auxiliary_queue = self.auxiliary_queue, self.main_queue

    def pop(self) -> int:

        # dequeues and returns the value representing the stack's top
        return self.main_queue.popleft()

    def top(self) -> int:

        # peeks at the stack's top without removing it
        return self.main_queue[0]

    def empty(self) -> bool:

        # returns True if the stack is empty exactly when the main queue is empty
